...
module.exports=async function handler(req,res){
  ...
  const lead={schemaVersion:"2.2",requestId:id,name,email,audience:enumValue(b.audience,ALLOWED.audience,"unknown"),city:text(b.city||"unknown",100),need:text(b.need,500),notes:text(b.notes,5000),timeframe:enumValue(b.timeframe,ALLOWED.timeframe,"flexible"),contactPreference:enumValue(b.contactPreference,ALLOWED.contactPreference,"email"),providerSelection:b.providerId||b.providerName?{id:b.providerId?text(b.providerId,150):null,name:b.providerName?text(b.providerName,300):null,provenance:"client-supplied-unverified-context"}:null,providerContext:sanitizeProviderContext(b.providerContext),sourcePage:text(b.sourcePage,500),partnerRoute:destination,accessConstraints,consentScope:"coordination-inquiry-partner-sharing",consentRecorded:true,consentRecordedAt:new Date().toISOString(),createdAt:new Date().toISOString()};
  ...
  try{const event={event:"qualified_healthcare_lead",schemaVersion:"2.2",platform:"japan-health",destination,lead},body=JSON.stringify(event);const r=await fetch(endpoint,{method:"POST",headers:{"Content-Type":"application/json","User-Agent":"Japan-Health-Lead-Handoff/2.2","X-Request-Id":id,"X-Japan-Health-Schema":"2.2",...signedHeaders(body,id)},body,signal:controller.signal});if(!r.ok)throw new Error(`Partner webhook returned ${r.status}`);return res.status(200).json({ok:true,forwarded:true,requestId:id});}
