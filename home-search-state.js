(()=>{
  ...
  function decorateCard(card){
    ...
    if(specialty&&!card.querySelector('.home-access-glance')){
      ...
      communication.textContent=languageSummary(card,p);
      const audience=document.createElement('span');
      audience.textContent=audienceSummary(p);
      glance.append(communication,audience);
    ...
    const location=card.querySelector('.fast-card-top>span:last-child');
    if(location)location.textContent=p.area||'Tokyo';
  }
