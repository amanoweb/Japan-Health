(()=>{
  ...
  // Capture phase guarantees neighborhood/station normalization happens before
  // home-search.js applies its hard area constraint. The selected constraint is
  // translated, never silently relaxed.
  ...
  function costLedger(p){
    const components=[
      ['Medical',p.medicalCost],
      ['Interpreter',p.interpreterCost],
      ['Coordinator',p.coordinatorCost]
    ];
    const recorded=components.filter(([,value])=>isRecorded(value)).length;
    return `<div class="home-cost-ledger" data-cost-ledger-home>
      <div class="home-cost-head"><div><small>TOTAL-COST VISIBILITY</small><b>${recorded}/3 components recorded</b></div><span>Data completeness only · not a quote</span></div>
      <div class="home-cost-components">${components.map(([label,value])=>`<div><span>${label}</span><strong class="${isRecorded(value)?'recorded':'verify'}">${isRecorded(value)?'Recorded':'Verify'}</strong></div>`).join('')}</div>
    </div>`;
  }
  ...
  function ensureHandoffDialog(){
    ...
    dialog.innerHTML=`<form method="dialog" class="home-handoff-shell" id="homeHandoffForm">
      ...
      <span class="home-handoff-kicker">JAPAN HEALTH ACCESS CHECK</span>
      <h2>Ask Japan Health to verify the access route.</h2>
      <p class="home-handoff-copy">We can pass the selected logistics context to a downstream coordination partner when configured. AMECA is currently a placeholder partner. This is not medical advice or a booking guarantee.</p>
      <div class="home-handoff-selected" id="homeHandoffSelected"></div>
      <label>Name<input id="homeHandoffName" autocomplete="name" required></label>
      <label>Email<input id="homeHandoffEmail" type="email" autocomplete="email" required></label>
      <label class="home-handoff-consent"><input id="homeHandoffConsent" type="checkbox" required><span>I consent to sharing this coordination inquiry with a Japan Health downstream partner if one is configured.</span></label>
      <button class="btn primary full" id="homeHandoffSubmit" type="submit">Send access verification inquiry</button>
      <p class="home-handoff-status" id="homeHandoffStatus" role="status" aria-live="polite"></p>
    </form>`;
    ...
    handoffForm.addEventListener('submit',async e=>{
      ...
      const provider=(window.PROVIDERS||[]).find(p=>String(p.id||'')===providerId);
      if(!provider)return;
      ...
      submit.disabled=true;status.textContent='Sending inquiry…';
      try{
        const response=await fetch('/api/lead',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
          ...
          costSnapshot:{
            recorded:[provider.medicalCost,provider.interpreterCost,provider.coordinatorCost].filter(isRecorded).length,
            total:3,
            missing:[['Medical',provider.medicalCost],['Interpreter',provider.interpreterCost],['Coordinator',provider.coordinatorCost]].filter(([,v])=>!isRecorded(v)).map(([k])=>k),
            completeness:[provider.medicalCost,provider.interpreterCost,provider.coordinatorCost].filter(isRecorded).length===3?'components-recorded':[provider.medicalCost,provider.interpreterCost,provider.coordinatorCost].filter(isRecorded).length?'partial':'needs-verification',
      ...
      }catch(error){status.textContent=error?.message||'Inquiry could not be sent. Please try again.';}
  ...
  function openHandoff(p){
    ...
    const s=currentSearch();
    dialog.dataset.providerId=String(p.id||'');
    dialog.querySelector('#homeHandoffSelected').innerHTML=`<b>${p.name}</b><span>${s.query||'Access verification'}${s.area?` · ${s.area}`:''}${s.audience!=='all'?` · ${s.audience}`:''}</span>`;
  ...
  function decorateCards(){
    grid.querySelectorAll('.fast-card').forEach(card=>{
      ...
      if(!card.querySelector('[data-booking-readiness-home]')){
      ...
      if(!card.querySelector('[data-cost-ledger-home]')){
        const readiness=card.querySelector('[data-booking-readiness-home]');
        readiness?.insertAdjacentHTML('afterend',costLedger(p));
      }
      const actions=card.querySelector('.fast-actions');
      if(actions&&!actions.querySelector('[data-access-handoff-home]')){
        const button=document.createElement('button');
        button.type='button';button.className='btn ghost';button.dataset.accessHandoffHome='true';button.textContent='Ask Japan Health to verify access';
        button.addEventListener('click',()=>openHandoff(p));
    ...
    if(summary&&area.dataset.enteredArea){
      ...
      if(summary.dataset.areaExplained!==entered){
        summary.textContent+=` “${entered}” was matched to ${canonical} for the area constraint.`;
        summary.dataset.areaExplained=entered;
      }
    }
  }
