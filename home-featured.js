(()=>{
  const root=document.getElementById("featuredTrack");if(!root)return;
  const providers=(window.PROVIDERS||[]).filter(p=>p&&p.city==="Tokyo"&&p.recordStatus==="official-source-verified"&&p.discoveryStatus!=="directory-only"&&!p.directoryBasis);
  function score(p){const direct={yes:100,partial:70,limited:35}[p.doctorEnglish]||20;const interp=["yes","available"].includes(p.interpreter)?90:p.interpreter==="external"?45:25;const reception=p.receptionEnglish==="yes"?100:p.receptionEnglish==="partial"?65:30;const docs=p.englishDocs==="yes"?100:p.englishDocs==="partial"?65:30;const communication=Math.round(direct*.45+interp*.30+reception*.15+docs*.10);const booking=Math.max(0,Math.min(100,85-(p.referral==="required"?30:p.referral==="varies"?12:0)-(p.coordinator==="required"?25:p.coordinator==="recommended"?10:p.coordinator==="varies"?8:0)));const eligibility=Math.max(0,Math.min(100,50+(p.selfPay==="yes"?20:0)+(Array.isArray(p.audience)&&p.audience.length>1?20:8)));const cost=({high:100,medium:70,low:35}[p.priceTransparency]||25);return Math.round(communication*.4+booking*.25+eligibility*.2+cost*.15);}
