(()=>{
  ...
  function serviceAccessState(matches,pRoute){
    ...
    if(confirmed.length){
      const e=confirmed[0],a=e.serviceAccess;
    ...
    if(attached.length){
      const e=attached[0],a=e.serviceAccess;
      return{state:"service-level-unverified",strength:2,label:`Service-level access needs verification · ${routeLabel[a.route]||a.route}`,detail:"A service-level access field exists, but it is not backed by an official source in the current record.",route:a.route,sourceUrl:null,verifiedDate:null,evidenceLabel:e.label};
    }
    if(pRoute.verified&&["direct-physician-english","interpreter","external-interpreter"].includes(pRoute.code)){
      return{state:"provider-level-only",strength:3,label:`Provider-level only · ${pRoute.label}`,detail:"Language access is documented for the provider, not for the matched specialist service. Confirm applicability before booking.",route:pRoute.code,sourceUrl:null,verifiedDate:null,evidenceLabel:null};
    }
  ...
  function snapshot(p,q){
    const matches=evidenceMatches(p,q),backed=matches.filter(e=>e.sourceBacked),specialty=specialtyMatch(p,q),r=providerRoute(p),serviceAccess=serviceAccessState(matches,r);
    ...
    return{query:String(q||"").trim(),evidenceState,evidenceStrength,sourceBackedCount:backed.length,matches:matches.slice(0,6),route:r,serviceAccess,providerProvenance:p?.recordStatus==="official-source-verified"?"official-source-checked":"demo-unverified",rank:evidenceStrength*100+serviceAccess.strength*18+r.strength*4+(p?.recordStatus==="official-source-verified"?10:0)};
  }
  ...
  function serviceBadge(s){
    if(s.serviceAccess.state==="service-level-confirmed")return '<span class="service-state confirmed">SERVICE-LEVEL CONFIRMED</span>';
    if(s.serviceAccess.state==="provider-level-only")return '<span class="service-state provider">PROVIDER-LEVEL ONLY</span>';
    if(s.serviceAccess.state==="service-level-unverified")return '<span class="service-state verify">SERVICE ACCESS · VERIFY</span>';
    return '<span class="service-state unknown">NEEDS VERIFICATION</span>';
  }
  function panel(p,q){
  ...
  function decorate(){
    ...
    try{
      ...
      const backed=snaps.filter(s=>s.sourceBackedCount>0).length,serviceConfirmed=snaps.filter(s=>s.serviceAccess.state==="service-level-confirmed").length,providerOnly=snaps.filter(s=>s.serviceAccess.state==="provider-level-only").length,needsVerification=snaps.filter(s=>["needs-verification","service-level-unverified"].includes(s.serviceAccess.state)).length;
      summary.innerHTML=`<b>${serviceConfirmed} service-level confirmed</b> for the searched disease/procedure · ${providerOnly} have provider-level language access only · ${needsVerification} need service-specific language verification · ${backed} have source-backed expertise evidence. Service-level confirmation is shown only when the matched service evidence carries its own official-source access record.`;
    }finally{busy=false;}
  }
  function ensureSort(){
    const sort=$("sort");if(!sort||[...sort.options].some(o=>o.value==="specialist-access"))return;
    const o=document.createElement("option");o.value="specialist-access";o.textContent="Service-level expertise + access evidence";sort.appendChild(o);
  }
  ...
  window.fetch=(input,init={})=>{
    try{
      ...
      if(url==="/api/lead"&&String(init.method||"GET").toUpperCase()==="POST"&&typeof init.body==="string"){
        ...
        if(snap&&q)body.specialistAccessSnapshot={query:q,evidenceState:snap.evidenceState,sourceBackedCount:snap.sourceBackedCount,matches:snap.matches.map(e=>({type:e.type,label:e.label,status:e.status,sourceUrl:e.sourceBacked?e.source:null,verifiedDate:e.sourceBacked?e.verified:null})),providerAccessRoute:snap.route.code,providerAccessRouteVerified:snap.route.verified,serviceAccess:{state:snap.serviceAccess.state,route:snap.serviceAccess.route,sourceUrl:snap.serviceAccess.sourceUrl,verifiedDate:snap.serviceAccess.verifiedDate,evidenceLabel:snap.serviceAccess.evidenceLabel},provenance:snap.providerProvenance,disclaimer:"Search-evidence and service-level language-route context only; not medical advice, clinical quality, outcomes, case acceptance, or a guarantee of current access."};
