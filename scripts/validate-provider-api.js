...
async function run(){
  ...
  await handler({method:"GET",url:"/api/providers"},jsonRes);
  ...
  await handler({method:"GET",url:"/api/providers?format=js"},jsRes);
  ...
  await handler({method:"POST",url:"/api/providers"},postRes);
  assert.equal(postRes.statusCode,405);
  console.log(`Provider API validation passed for ${payload.providers.length} records.`);
}
