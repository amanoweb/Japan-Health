...
async function run(){
  ...
  for(const id of ["jn-handoff-american-clinic-tokyo","jn-handoff-kishi-clinica-femina","jn-handoff-minamiaoyama-eye"]){const p=payload.providers.find(x=>x.id===id);assert.ok(p,`${id} must be present after provider promotion`);assert.equal(p.discoveryStatus,"provider-level-verified");assert.equal(p.recordStatus,"official-source-verified");assert.ok(/^https:\/\//.test(p.source));assert.ok(Array.isArray(p.expertiseEvidence)&&p.expertiseEvidence.length>0);}
